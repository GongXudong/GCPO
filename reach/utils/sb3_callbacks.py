...
class EmaBCPolicyCallback(BaseCallback):

    def __init__(self, ema_gamma: float = 0.99, ema_freq: int = 100, verbose: int = 0):
        super().__init__(verbose)
        self.ema_gamma = ema_gamma
        self.ema_freq = ema_freq

    def _on_step(self) -> bool:
        if self.n_calls % self.ema_freq == 0:
            polyak_update(self.model, self.globals["bc_trained_algo"], 1-self.ema_gamma)
        return True
