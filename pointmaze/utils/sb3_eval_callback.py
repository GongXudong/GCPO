...
from utils.sb3_evaluate_policy import evaluate_policy_with_stat
...
class MyEvalCallbackSTAT(EventCallback):
    ...
    def __init__(
        ...
        success_stat_list: List = [],  # [{'goal': [], 'weight': 0.1}]
        success_stat_annealing_coef: float = 0.99,
        success_stat_list_max_length: int = 1000,
        ...
        self.success_stat_list = success_stat_list
        self.success_stat_annealing_coef = success_stat_annealing_coef
        self.success_stat_list_max_length = success_stat_list_max_length
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # 先衰减
            if self.success_stat_annealing_coef != 1.0:
                for item in self.success_stat_list:
                    item["weight"] *= self.success_stat_annealing_coef

            for item in stat_dict_arr:
                if item["success"]:
                    self.success_stat_list.append({
                        "goal": item["desired_goal"],
                        "weight": 1.0
                    })

            # 维护success_stat_list的长度
            if len(self.success_stat_list) > self.success_stat_list_max_length:
                self.success_stat_list = self.success_stat_list[-self.success_stat_list_max_length:]
            ...
            if self.sync_success_stat:
                ...
                self.training_envs.env_method(self.sync_success_stat_env_method_name, self.success_stat_list)
